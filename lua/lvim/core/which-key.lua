...
M.config = function()
  lvim.builtin.which_key = {
    ...
    -- New which-key spec format (flat list)
    vmappings = {
      {
        mode = { "v" },
...
M.setup = function()
  ...
  -- New which-key spec format uses flat list, register directly
  local mappings = lvim.builtin.which_key.mappings
  local vmappings = lvim.builtin.which_key.vmappings

  -- Register mappings using the new flat format
  -- The new format is a flat list that can be registered directly
  which_key.register(mappings)
  
  -- Visual mode mappings are grouped by mode
  for _, mapping_group in ipairs(vmappings) do
    which_key.register(mapping_group)
  end
